...
<?php  
    ...
    if (isset($_GET['search'])) {
        ...
        $search = "%{$_GET['search']}%";
        //มันจะดึงข้อมูลมาจาก news title กับ news intro
        $stmt = $conn->prepare("SELECT * FROM tbl_news WHERE news_title LIKE ? OR news_intro LIKE ?");
        // เราดึงข้อมูลมาจาก 2 ที่ เลยต้องมี $search 2ตัว
        $stmt->execute([$search , $search]);
        $result = $stmt->fetchAll(); //ดึงข้อมูลมาแสดงข้อมูลทั้งหมด

        //ถ้าเจอข้อมูลมากกว่า 0 ให้ loop
        if ($stmt->rowCount() > 0) {
...
<div class="container">
  <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="search.php" style="color: black;">ค้นหา</a></li>
  ...
  <div class="row">
    <div class="col-md-4">
                    <div class="card mb-4 shadow-sm">
                        <?php if (!empty($rows['news_img'])) { ?>
                            <a href="news_inside.php?id=<?= $rows['news_id'] ?>">
                                <img src="upload_img/<?php echo $rows['news_img'] ?>" width="348" />
                            </a>
                        <?php } else { ?>
                            <img src="assets/img/no-img.png" width="348" alt="news_img">
                        <?php } ?>

                        <div class="card-body">
                            <p class="text"><h4><?php echo $rows['news_title'] ?></h4></p>
                            <p class="text-card"><h9><?php echo $rows['news_intro'] ?></h9></p>
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="btn-group">
                                    <a href="news_inside.php?id=<?= $rows['news_id'] ?>" button type="button" class="btn btn-sm btn-outline-secondary">View</a>
                                </div>
                            </div>
                        </div>
                    </div>
